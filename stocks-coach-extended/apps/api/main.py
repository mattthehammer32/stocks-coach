...
def news_fetch(symbol: str):
    # TODO: Replace with Polygon/NewsAPI. For now we seed one fake item per call (idempotent-ish).
    sym = symbol.upper()
    now = int(pd.Timestamp.utcnow().timestamp())
    items = NEWS_CACHE.setdefault(sym, [])
    # Dedup by title
    title = f"{sym} placeholder headline"
    if not any(x["title"]==title for x in items):
        items.insert(0, {
            ...
            "source": "placeholder",
            "summary": "This is a stub. Wire a real news source and LLM summarizer for impact.",
            "impact": "neutral",
            "confidence": 0.2
        })
        NEWS_CACHE[sym] = items[:50]
